...
function CartToast({ id, item }: { id: string | number; item: ToastItem }) {
  const t = useT();
  return (
      ...
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="grid h-6 w-6 place-items-center rounded-full bg-store-accent/15">
            <CheckCircle2 className="h-3.5 w-3.5 text-store-accent" strokeWidth={2.5} />
          </span>
...
export function showCartToast(item: ToastItem) {
  toast.custom((id) => <CartToast id={id} item={item} />, {
    duration: 4500,
    position: "top-right",
    unstyled: true,
  });
